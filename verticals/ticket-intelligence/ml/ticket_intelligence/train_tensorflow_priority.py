"""Train a TensorFlow/Keras priority classifier on the fixed ticket splits."""
...
def load_split(path: Path) -> pd.DataFrame:
    ...
    if "priority" not in df.columns and "true_priority" not in df.columns:
        raise ValueError(f"{path} must include priority or true_priority.")
    if "priority" not in df.columns:
        df["priority"] = df["true_priority"]
    df[INPUT_FEATURE] = df[INPUT_FEATURE].fillna("").astype(str).str.strip()
    df["priority"] = df["priority"].fillna("").astype(str).str.strip().str.lower().str.replace(" ", "_")
    df = df[(df[INPUT_FEATURE].str.len() > 0) & (df["priority"].str.len() > 0)].copy()
    return df
...
def build_label_mapping(train_labels: pd.Series) -> dict[str, Any]:
    ...
    if len(labels) < 2:
        raise ValueError("Need at least two priority labels to train a classifier.")
...
def build_model(tf, num_classes: int):
    ...
    model = tf.keras.Sequential(
        [
            ...
            tf.keras.layers.Dense(num_classes, activation="softmax", name="priority_softmax"),
        ],
        name="opspilot_tensorflow_priority_classifier",
    )
...
def write_confusion_matrix_png(labels: list[str], matrix: list[list[int]], output_path: Path) -> None:
    ...
    ax.set(
        ...
        title="TensorFlow Priority Confusion Matrix",
    )
...
def train_tensorflow_priority(epochs: int = DEFAULT_EPOCHS, batch_size: int = BATCH_SIZE) -> dict[str, Any]:
    ...
    label_mapping = build_label_mapping(train_df[LABEL_COLUMN])
    label_to_id = label_mapping["label_to_id"]
    labels = label_mapping["labels"]

    y_train = encode_labels(train_df[LABEL_COLUMN], label_to_id, "train")
    y_val = encode_labels(val_df[LABEL_COLUMN], label_to_id, "validation")
    y_test = encode_labels(test_df[LABEL_COLUMN], label_to_id, "test")
    ...
    print(f"Building model for {len(labels)} priority classes...", flush=True)
    ...
    print("Training TensorFlow priority model...", flush=True)
    history = model.fit(
        train_texts,
        y_train,
    ...
    metrics = {
        "model_version": "ticket-intelligence-tensorflow-priority-v1",
        ...
        "data": {
            ...
            "label": LABEL_COLUMN,
            "label_quality_note": "Priority is supervised from source labels but may be noisy or context-dependent.",
        },
        "row_counts": {
    ...
    label_mapping_path = ARTIFACT_DIR / "tensorflow_priority_label_mapping.json"
    label_mapping_path.write_text(json.dumps(label_mapping, indent=2), encoding="utf-8")
    model_path = ARTIFACT_DIR / "tensorflow_priority_model.keras"
    print(f"Saving TensorFlow model to {model_path}...", flush=True)
    model.save(model_path)

    metrics_path = OUTPUT_DIR / "tensorflow_priority_metrics.json"
    metrics_path.write_text(json.dumps(metrics, indent=2), encoding="utf-8")
    pd.DataFrame(report).T.to_csv(OUTPUT_DIR / "tensorflow_priority_classification_report.csv")
    write_confusion_matrix_png(labels, matrix, OUTPUT_DIR / "tensorflow_priority_confusion_matrix.png")

    base_error_columns = [column for column in ["ticket_id", "external_id", "customer_message", INPUT_FEATURE, LABEL_COLUMN] if column in test_df.columns]
    error_df = test_df[base_error_columns].copy()
    error_df["true_priority"] = error_df[LABEL_COLUMN]
    error_df["predicted_priority"] = predicted_labels
    error_df["priority_confidence"] = [round(float(value), 4) for value in confidences]
    error_df["priority_correct"] = error_df["true_priority"] == error_df["predicted_priority"]
    error_df = error_df[~error_df["priority_correct"]].copy()
    error_df.to_csv(OUTPUT_DIR / "tensorflow_priority_error_analysis.csv", index=False)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a TensorFlow/Keras priority classifier for Ticket Intelligence.")
    parser.add_argument("--epochs", type=int, default=DEFAULT_EPOCHS)
    parser.add_argument("--batch-size", type=int, default=BATCH_SIZE)
    args = parser.parse_args()
    summary = train_tensorflow_priority(epochs=args.epochs, batch_size=args.batch_size)
